...
| **Document AI / OCR** | Disabled Mode (Optional Integration) | `IMPLEMENTED` | `OCR_PROVIDER=disabled` | None (Allows startup without cloud OCR) | Automatic OCR extraction unavailable; RFC7807 response |
| **Document AI / OCR** | Google Cloud Document AI | `IMPLEMENTED` | `GOOGLE_DOC_AI_PROJECT_ID`, `GOOGLE_DOC_AI_LOCATION`, `GOOGLE_DOC_AI_PROCESSOR_ID` | Google Cloud Paid Project with Document AI API & Custom Processor enabled | `MockOcrProvider` (Deterministic catalog matching in dev) |
| **Document AI / OCR** | AWS Textract | `IMPLEMENTED` | `AWS_TEXTRACT_REGION`, `AWS_TEXTRACT_ACCESS_KEY_ID`, `AWS_TEXTRACT_SECRET_ACCESS_KEY` | AWS Account with Amazon Textract IAM permissions | `MockOcrProvider` (Deterministic catalog matching in dev) |
...
| **Maps & Geospatial Routing** | Disabled Mode (Optional Integration) | `IMPLEMENTED` | `MAPS_PROVIDER=disabled` | None (Allows startup without Google Maps / Mapbox) | Transparent geodesic Haversine distance (`mode: 'GEODESIC_FALLBACK'`, `is_fallback: true`) |
| **Maps & Geospatial Routing** | Google Maps Platform | `IMPLEMENTED` | `GOOGLE_MAPS_API_KEY` | Google Cloud Paid Project with Distance Matrix & Geocoding APIs enabled | `MockDistanceProvider` / Geodesic Haversine fallback |
| **Maps & Geospatial Routing** | Mapbox Directions / Geocoding | `IMPLEMENTED` | `MAPBOX_ACCESS_TOKEN` | Mapbox Account & Access Token | `MockDistanceProvider` / Geodesic Haversine fallback |
